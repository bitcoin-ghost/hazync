...
#pragma unroll
    for (uint32_t j = 0; j < CELLS_RATE; j++, i++)
      // (size_t) — upstream risc0#3773 / risc0#3781. `i` (column) and `dim_x` (domain) are both
      // uint32_t, so `i * dim_x` runs in 32 bits and wraps once column*domain crosses 2^32. The
      // kernel then reads the wrong cell and commits a hash over the wrong data, with NO error:
      // the GPU returns a valid-looking seal that fails verification.
      //
      // The threshold halves with every po2 step -- ~256 columns at po2 22, ~128 at po2 23 -- so
      // rv32im is under it at po2 22 and (we believe) over it at po2 23, where we measured a
      // deterministic failure on segment 0, 3 attempts of 3. See docs/ACCELERATION.md.
