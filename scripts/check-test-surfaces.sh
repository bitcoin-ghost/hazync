...
declare -A EXCLUDED=(
  ...
  [hazed-chain-verify.py]="needs a live mainnet bitcoind (#31 Phase 1). Demonstrates that merkle-root identity composes with Hazync validity for hazed blocks. Run by hand; it refuses --txid-source gsb with exit 1 rather than silently falling back, so it cannot report a pass for an untested path."
