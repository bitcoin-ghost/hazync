> **Historical record.** Two things here are superseded: #119 was fixed in v0.21.1 (#245), and the retry now
> lives in the binary — `prove-chunk` and `agg-chunks` retry on #119 since #240 (v0.21.1) — so the external
> wrapper and the 6.3% fault rate describe the v0.21.0 binary. Current multi-GPU runbook: `../FLEET_OPERATIONS.md`.
...
the blocks run 966,107 > 966,106 > 966,108; ordered by chunk time they run 966,108 > 966,107 > 966,106 —
no consistent relation. *(2026-09-14: this previously said "exactly the reverse", which the table does not show.)*
...
which is the only reason to trust it at all. *(2026-09-14: rows recomputed from the formula — N=13 gives
569 s = 9.49 min and N=10 gives 12.24 min; they previously read ~9.8 and 12.3.)*
...
execute is removed from the critical path by hazync#236 (merged 2026-09-08 16:58 UTC, before these runs, but not in the v0.21.0 binary
they used), worth ~2% at N=8
and more as N grows.
