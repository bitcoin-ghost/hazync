...
> **Historical record, archived here on 2026-09-14** from `f24c43b:docs/TOPOLOGY_AND_SETTINGS.md` (last changed
> 2026-09-05, #200). Superseded by [`../TOPOLOGY_AND_SETTINGS.md`](../TOPOLOGY_AND_SETTINGS.md), rewritten for the
> CORE guest in #308 — what is current: CORE fleet sizing there (§1.1, ~13 L40S, inferred from an 8-card fleet)
> and the shipped build settings (§4). This revision priced the stock guest and #139, which do not ship; its
> §0.5 carries the 2-card aggregate measurement (1.81x) that `TEN_MINUTE_BLOCK.md` cites.
