...
either way. **Releases:** the current release is **v0.8.0** at `METHOD_ID 7a8b29e0` (real-Core `pow.cpp`
...
the same guest ELF — so the CPU-only `reproduce/Dockerfile` attests the canonical id (`7a8b29e0`,
