...
# the card that rebuilds the receipt: any GPU box that took no part in the run
SIP=${HAZYNC_REGEN_IP:?set HAZYNC_REGEN_IP}; SPORT=${HAZYNC_REGEN_PORT:?set HAZYNC_REGEN_PORT}
...
#    (hazync.org/repro/) 404ed from at least 2026-09-18 until it was restored on 2026-09-21 --
#    the migration to hazync.org had left the fixtures behind on bitcoinghost.org. They are
#    back and verified byte-identical. block_966256.json is still not committed,
...
case "$PREP" in
  *ready*) : ;;
  *) say "REGEN CARD NOT READY: ${PREP:-no output}"; exit 1 ;;
esac
