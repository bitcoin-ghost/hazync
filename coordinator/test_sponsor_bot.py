...
#351: the bot has no access to the coordinator's database. It talks to the REAL coordinator request handler,
served over HTTP here, through the signed bot API, and keeps its cost log in a database of its own. Only the
test itself (and the fake pods, standing in for proofs landing) write the coordinator's database.
...
# The bot's API key, and its public half where the coordinator reads it.
from cryptography.hazmat.primitives.asymmetric.ed25519 import Ed25519PrivateKey  # noqa: E402
from cryptography.hazmat.primitives import serialization as _ser  # noqa: E402
_KEYDIR = tempfile.mkdtemp(prefix="botkey_")
_sk = Ed25519PrivateKey.generate()
BOT_KEY_FILE = os.path.join(_KEYDIR, "bot.key")
with open(BOT_KEY_FILE, "w") as _f:
    _f.write(_sk.private_bytes(_ser.Encoding.Raw, _ser.PrivateFormat.Raw, _ser.NoEncryption()).hex())
with open(os.path.join(_KEYDIR, "bot.pub"), "w") as _f:
    _f.write(_sk.public_key().public_bytes(_ser.Encoding.Raw, _ser.PublicFormat.Raw).hex())
os.environ["SPONSOR_BOT_PUBKEY_FILE"] = os.path.join(_KEYDIR, "bot.pub")
...
_coord_httpd = server.ThreadingHTTPServer(("127.0.0.1", 0), server.H)
threading.Thread(target=_coord_httpd.serve_forever, daemon=True).start()
COORD_URL = f"http://127.0.0.1:{_coord_httpd.server_address[1]}"
COORD = sponsor_bot.Coord(COORD_URL, open(BOT_KEY_FILE).read())
BOTDB = os.path.join(tempfile.mkdtemp(prefix="botdb_"), "bot.db")      # not created until the bot writes its log
os.environ.update(SPONSOR_BOT_DB=BOTDB, SPONSOR_BOT_KEY_FILE=BOT_KEY_FILE, SPONSOR_BOT_COORD_URL=COORD_URL)
if CONTROL:
...
class FakeRunner:
    ...
    def start(self, pod, work):
        ...
        sids = {r["sponsorship_id"] for r in qb("SELECT sponsorship_id FROM sponsor_work WHERE pod_id=? AND outcome IS NULL"
                                                " AND sponsorship_id IS NOT NULL", (pod.id,))}
        self.status_at_start.setdefault(pod.id, []).append(sorted({r[0] for r in c.execute(
            f"SELECT status FROM sponsorships WHERE id IN ({','.join('?' * len(sids))})", tuple(sids))}))
        c.close()
...
def make_bot(api, runner, **kw):
    ...
    args.update(kw)
    args.setdefault("coord", COORD)
    return sponsor_bot.Bot(BOTDB, sponsor_bot.RunPod("test-key", url=api.url, timeout=10), runner, **args)
...
def qb(sql, args=()):
    ...
    c = sqlite3.connect(BOTDB, timeout=30)
    c.row_factory = sqlite3.Row
    try:
        rows = c.execute(sql, args).fetchall()
        c.commit()
...
def reset():
    ...
    for path, tables in ((DB, ("sponsorships", "vranges", "ranges", "sponsor_keys")), (BOTDB, ("sponsor_work", "sponsor_pods"))):
...
check(not os.path.exists(BOTDB), "there is no cost log before the bot has run")
check(sponsor_bot.main([]) == 0, "plan (the default) runs, through the coordinator's API")
check(not os.path.exists(BOTDB) and not q("SELECT name FROM sqlite_master WHERE name IN ('sponsor_work','sponsor_pods')"),
      "plan wrote nothing: no cost log, and nothing added to the coordinator's database")
...
for kw, why in ((dict(max_pods=0), "zero pods"), (dict(max_usd=0), "a zero total cap"),
    ...
    args.update(kw)
    try:
        sponsor_bot.Bot(BOTDB, None, None, **args)
        refused = False
...
got = sponsor_bot.pending_work(COORD)
...
proof(21, 21)
got = sponsor_bot.pending_work(COORD, busy={10})
check(got == [(b, 20), (f, 30), (a, 11)], f"proven and busy heights are skipped ({got})")
check([r["id"] for r in sponsor_bot.queue(COORD)] == [b, f, a], "queue() lists the same holds, for test_block_api's plan")
...
proof(90, 90)
proof(100, 101)
done = sponsor_bot.reconcile(COORD)
...
sponsor(502, 503)
bad = sponsor_bot.trial_refusals(COORD, [500, 501, 502, 503, 504, 505])
...
rows = qb("SELECT sponsorship_id, height, outcome FROM sponsor_work ORDER BY height")
...
reset()
bad = sponsor_bot.trial_refusals(COORD, [505, 400000])
...
work = qb("SELECT * FROM sponsor_work")
...
pods = qb("SELECT * FROM sponsor_pods")
...
c = sponsor_bot.connect(BOTDB)
sponsor_bot.ensure_tables(c)
bot._launch(c, bot.clock())
c.close()
note = (qb("SELECT note FROM sponsor_pods") or [{"note": ""}])[0]["note"] or ""
...
for delay, what in ((0.0, "a fast API"), (0.03, "an API that takes 30 ms to answer")):
    ...
    check(qb("SELECT COUNT(*) AS n FROM sponsor_work WHERE outcome IS NULL")[0]["n"] == 0, f"no work row is left in flight ({what})")
...
stalled = qb("SELECT height FROM sponsor_work WHERE pod_id=? AND outcome='stalled' ORDER BY height", (first,))
...
first = runner.booted[0]
note = qb("SELECT note FROM sponsor_pods WHERE pod_id=?", (first,))[0]["note"] or ""
...
check(qb("SELECT COUNT(*) n FROM sponsor_work WHERE outcome='failed'")[0]["n"] == 4,
      "its four blocks are logged as failed on that pod, then proven on the next")
check(qb("SELECT note FROM sponsor_pods WHERE note LIKE 'could not start work%'"), "and the pod's note says why")
...
first = runner.booted[0]
rows = qb("SELECT height, outcome, seconds FROM sponsor_work WHERE pod_id=? ORDER BY height", (first,))
...
for vid, h, key, ts in (("e", 90005, K, 1020), ("a", 90000, K, 1040), ("b", 90001, K, 1058),
    ...
    q("INSERT INTO vranges(id, lo, hi, pubkey, handle, ts) VALUES(?,?,?,?,?,?)", (vid, h, h, key, "h", ts))
buf = io.StringIO()
with contextlib.redirect_stdout(buf):
    code = sponsor_bot.main(["report"])
out = buf.getvalue()
got = {r["height"]: r for r in qb("SELECT * FROM sponsor_work")}
...
c = sponsor_bot.connect(BOTDB)
check(sponsor_bot.reconcile_work(c, COORD) == 0, "a second pass changes nothing")
c.close()
...
rows = qb("SELECT sponsorship_id, height, pubkey FROM sponsor_work")
...
_other = Ed25519PrivateKey.generate().private_bytes(_ser.Encoding.Raw, _ser.PrivateFormat.Raw, _ser.NoEncryption()).hex()
api = FakeRunPod()
try:
    make_bot(api, FakeRunner(), trial=[9400], coord=sponsor_bot.Coord(COORD_URL, _other)).run()
    refused = False
except sponsor_bot.BotRefused as e:
    refused = "403" in str(e)
check(refused and not api.deploys, "the bot refuses to rent anything when the coordinator does not accept its API key")
try:
    make_bot(api, FakeRunner(), trial=[9400], coord=sponsor_bot.Coord("http://127.0.0.1:9", _other, timeout=2)).run()
    refused = False
except sponsor_bot.BotRefused:
    refused = True
check(refused and not api.deploys, "or when it cannot reach the coordinator at all")
try:
...
# ---------- #351: the coordinator's API, not its database ----------
print("== the bot and the coordinator agree on the signed message ==")
for args in (("GET", "/api/bot/blocks?h=1,2", "1789000000", "ab" * 16, b""),
             ("POST", "/api/bot/key", "1789000001", "cd" * 16, b'{"pubkey": "x"}')):
    check(sponsor_bot.Coord.message(*args) == server.bot_message(*args), f"the same bytes are signed for {args[0]} {args[1]}")
src = open(sponsor_bot.__file__).read()
check("COORD_DB" not in src and not re.search(r"(FROM|INTO|UPDATE)\s+(sponsorships|vranges|ranges|sponsor_keys)\b", src),
      "sponsor_bot.py names no coordinator table in SQL and never opens COORD_DB")


class FlakyCoord(sponsor_bot.Coord):
    """The real API, except calls number `fail_from` to `fail_to` (counted from 1) fail as a coordinator restart would."""

    def __init__(self, fail_from, fail_to):
        super().__init__(COORD_URL, open(BOT_KEY_FILE).read())
        self.n, self.fail_from, self.fail_to = 0, fail_from, fail_to

    def _call(self, method, path, body=None):
        self.n += 1
        if self.fail_from <= self.n <= self.fail_to:
            raise sponsor_bot.CoordError("simulated: the coordinator is restarting")
        return super()._call(method, path, body)


print("== a coordinator restart does not throw the pods away ==")
reset()
s_blip = sponsor(9600, 9609)
api = FakeRunPod()
logs = []
blip = FlakyCoord(12, 14)                 # three calls fail, once pods are up and working
reason = make_bot(api, FakeRunner(per_block=0.02), coord=blip, max_pods=2, blocks_per_pod=5, log=logs.append).run()
check(any("failed this look" in m for m in logs), f"the failures happened mid-run ({blip.n} calls)")
check(reason == "done" and status_of(s_blip) == "proven" and not api.live(),
      f"a short outage is ridden out: the run finishes and the sponsorship is proven (reason {reason}, {status_of(s_blip)})")

print("== a coordinator that stays away stops the run, with every pod terminated ==")
reset()
sponsor(9650, 9699)
api = FakeRunPod()
down = FlakyCoord(12, 10 ** 9)
reason = make_bot(api, FakeRunner(per_block=0.5), coord=down, max_pods=2, blocks_per_pod=10).run()
check(reason == "coordinator" and api.deploys and api.deployed() <= set(api.terminations) and not api.live(),
      f"the run stops as 'coordinator' and terminates every pod it rented (reason {reason}, live {api.live()})")
check(qb("SELECT COUNT(*) AS n FROM sponsor_work WHERE outcome IS NULL")[0]["n"] == 0,
      "no work row is left in flight, though the coordinator could not say which blocks landed")

print("== import-log copies the old cost log out of a coordinator database ==")
old = os.path.join(tempfile.mkdtemp(prefix="oldcoord_"), "coordinator.db")
oc = sqlite3.connect(old)
sponsor_bot.ensure_tables(oc)
oc.execute("INSERT INTO sponsor_work(id, height, pod_id, outcome, seconds, usd_estimate, pubkey) VALUES(7, 90002, 'p1', 'proven', 20, 0.004, 'k')")
oc.execute("INSERT INTO sponsor_pods(pod_id, name, terminated_at, usd_estimate) VALUES('p1', 'hz-sponsor-1-1', 5, 0.3)")
oc.commit()
oc.close()
reset()
...
with _cl.redirect_stdout(out):
    code1 = sponsor_bot.main(["import-log", old])
    code2 = sponsor_bot.main(["import-log", old])
check(code1 == code2 == 0 and "imported 1 work row(s) and 1 pod(s)" in out.getvalue()
      and "imported 0 work row(s) and 0 pod(s)" in out.getvalue(), f"rows are copied once ({out.getvalue().splitlines()})")
check([tuple(r) for r in qb("SELECT id, height, outcome FROM sponsor_work")] == [(7, 90002, "proven")]
      and qb("SELECT usd_estimate FROM sponsor_pods WHERE pod_id='p1'")[0]["usd_estimate"] == 0.3, "with their values")
reset()
...
_saved_key = os.environ.pop("SPONSOR_BOT_KEY_FILE")
_buf = io.StringIO()
with contextlib.redirect_stdout(_buf):
    _code = sponsor_bot.main(["report"])
os.environ["SPONSOR_BOT_KEY_FILE"] = _saved_key
check(_code == 0 and "MEASURED" in _buf.getvalue() and "nothing was corrected" in _buf.getvalue(),
      "report still reports from the bot's own log when it cannot reach the coordinator")
