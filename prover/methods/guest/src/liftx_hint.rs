...
//!
//! ⛔ **2026-08-31: the 9.83% below is STALE and overstates this lever ~6x.** It was measured with
//! libsecp's SOFTWARE field arithmetic. With `patches/0012`'s coprocessor field backend, every
//! multiply and square inside the sqrt chain is already ~14x cheaper, so re-measured on the same
//! block, `secp256k1_fe_sqrt` is **241,023,621 cycles CUMULATIVE = 6.17%** — not 1.42 G / 9.83%.
//! Whatever this saves, it is bounded by 6.17%. Keep the original figure for the un-accelerated
//! case, but do not quote it for a build that carries the field backend.
//!
//! ⚠ Its FLAT self-time is 0.30%. A flat profile hides this lever 20x, because the ~270 chain
//! operations are attributed to the coprocessor wrappers, not to `fe_sqrt`. Size it cumulatively.
//!
