...
```
raw       1,065,400 bytes
gzipped     295,130 bytes   ← what a browser actually downloads (`gzip -c < FILE`; passing the path
...
will think the second is broken. The 21 ms is a Groth16-wrapped range; the 254 ms is the kind of STARK
receipt `/api/spine/proof` serves, which is the file most people will drop on the page. The spine has
grown far past block 1,789 since that measurement (227,746 B for [1..24,827] on 2026-09-14) and the
browser figure has not been re-measured.
...
is 63 lines of glue in `hazync-verify.js`, short enough to read in full before running it.
...
Serve the `.wasm` with `Content-Encoding: gzip` — it compresses 3.6× (1,065,400 → 295,130 B with
